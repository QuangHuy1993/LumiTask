...
import { deleteSubscriptionAction } from "@/features/trading/actions/subscriptionActions";

type SubscriptionDeleteModalProps = {
  open: boolean;
...
export function SubscriptionDeleteModal({ open, subscriptionId, title, onClose, onDeleted }: SubscriptionDeleteModalProps) {
  ...
  if (!mounted || !open || subscriptionId == null) return null;
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (!res.success) {
        if (res.error === "NOT_FOUND") toast.error("Không tìm thấy dịch vụ.");
        else if (res.error === "UNAUTHENTICATED") toast.error("Bạn cần đăng nhập lại.");
        else toast.error("Không thể xoá dịch vụ.");
        return;
      }
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md rounded-2xl border border-moss-100 bg-white p-6 shadow-card">
        <div className="flex items-start justify-between gap-3">
          <h2 className="text-lg font-bold text-moss-900">Xoá dịch vụ?</h2>
        ...
        <p className="mt-3 text-sm text-moss-600">
          <span className="font-bold text-moss-900">{title}</span> sẽ bị ẩn khỏi danh sách (soft delete).
        </p>
